...
Output format is TSV:
  external_id<TAB>v0,v1,...,v511

Default model is BAAI/bge-small-en-v1.5 (384 dims). Vectors are L2-normalized
and padded/truncated to MoonShot's fixed 512-dim DocData vector slot.
"""
...
def write_vectors(rows, output: Path, model_name: str, batch_size: int, max_length: int, id_field: str, text_fn, output_format: str, id_bytes: int, text_prefix: str):
    ...
    with output.open(mode, **open_kwargs) as out:
        if output_format == "i8bin":
            ...
            out.write(struct.pack("<II", 512, id_bytes))
        batch_ids = []
        batch_texts = []
        for row in rows:
            ...
            if len(batch_ids) >= batch_size:
                done += flush_batch(model, batch_ids, batch_texts, out, output_format, id_bytes)
...
def flush_batch(model, batch_ids, batch_texts, out, output_format: str, id_bytes: int):
    ...
    if embeddings.shape[1] < 512:
        padded = np.zeros((embeddings.shape[0], 512), dtype=np.float32)
