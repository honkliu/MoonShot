...
float DocDataScore(const DocDataEntry& entry)
{
    ...
    return g_ScoringParameters.QMP_StaticWeight * DocDataDecodeScore(entry.DDE_StaticRank)
        + g_ScoringParameters.QMP_PriorWeight * DocDataPrior(entry)
        + g_ScoringParameters.QMP_QualityWeight * DocDataDecodeScore(entry.DDE_QualityScore)
        + g_ScoringParameters.QMP_AuthorityWeight * DocDataDecodeScore(entry.DDE_AuthorityScore)
        - g_ScoringParameters.QMP_SpamPenalty * DocDataDecodeScore(entry.DDE_SpamScore);
}

float VectorScoreFeature(const DocDataEntry& entry, const std::vector<float>* query)
{
    if (!query || query->size() != DOC_VECTOR_DIM || entry.DDE_VectorDim != DOC_VECTOR_DIM || entry.DDE_VectorFormat == 0)
        return 0.0f;
