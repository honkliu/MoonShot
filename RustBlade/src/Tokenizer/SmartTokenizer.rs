//! Direct translation of the C++ smart tokenizer; symbol names stay aligned for debugging.
...
impl SmartTokenizer {
    ...
    fn IsIndexableToken(token: &str) -> bool {
        ...
        if token.bytes().any(|ch| ch < 0x20 || ch == 0x7f) {
            return false;
        }
...
fn lowercase_for_locale(text: &str, locale: &str) -> String {
    ...
    let turkic = locale.eq_ignore_ascii_case("tr")
        || locale.eq_ignore_ascii_case("tr-tr")
        || locale.eq_ignore_ascii_case("az")
        || locale.eq_ignore_ascii_case("az-az");
...
fn is_vowel(ch: u8) -> bool {
    matches!(ch, b'a' | b'e' | b'i' | b'o' | b'u' | b'y')
}
...
fn region_after_vowel_consonant(
    word: &[u8],
    marked_y: &[bool],
    start: usize,
    minimum: usize,
...
fn step_1b(word: &mut String, r1: usize, original_marked_y: &[bool]) {
    ...
    if !contains_vowel(word.as_bytes(), stem_length, original_marked_y) {
        return;
    }
...
fn step_3(word: &mut String, r1: usize, r2: usize) {
    ...
    if suffix_in_region(word, "ative", r2) {
        replace_suffix(word, "ative", "");
    }
}
...
fn step_4(word: &mut String, r2: usize) {
    ...
    if suffix_in_region(word, "ion", r2) {
        ...
        if matches!(preceding, b's' | b't') {
            replace_suffix(word, "ion", "");
        }
...
fn step_5(word: &mut String, r1: usize, r2: usize, original_marked_y: &[bool]) {
    if word.ends_with('e') {
        ...
        if e_position >= r2 {
            ...
            if !is_short_syllable(stem, marked) {
                word.pop();
            }
        }
...
mod tests {
    ...
    fn stems_english_with_snowball_rules() {
        let tokenizer = SmartTokenizer::new();
        assert_eq!(
            tokenizer.Tokenize("suggested suggests suggesting"),
            vec!["suggest", "suggest", "suggest"]
        );
    }
}
