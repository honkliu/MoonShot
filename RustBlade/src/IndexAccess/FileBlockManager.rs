//! Direct translation of the C++ file block manager; symbol names stay aligned for debugging.
...
impl FileBlockManager {
    ...
    pub fn openMemory(&mut self, memory: Arc<[u8]>) -> bool {
        if memory.len() < self.m_BlockSize {
            return false;
        }
        self.m_FileAccess = None;
    ...
    pub fn read(&self, block_seq: u32, buffer: &mut [u8]) -> bool {
        if buffer.len() < self.m_BlockSize {
            return false;
        }
        if let Some(memory) = self.m_Memory.as_ref() {
            let offset = block_seq as usize * self.m_BlockSize;
            let Some(source) = memory.get(offset..offset + self.m_BlockSize) else {
                return false;
            };
        ...
        self.m_FileAccess
            .as_ref()
            .map(|file| file.ReadBlock(block_seq, buffer, self.m_BlockSize, self.m_BaseOffset))
            .unwrap_or(false)
    }
    ...
    pub fn setBlockSize(&mut self, block_size: usize) {
        self.m_BlockSize = block_size;
    }
    pub fn getBaseOffset(&self) -> u64 {
        self.m_BaseOffset
    }
    pub fn setBaseOffset(&mut self, base_offset: u64) {
        self.m_BaseOffset = base_offset;
    }
}
