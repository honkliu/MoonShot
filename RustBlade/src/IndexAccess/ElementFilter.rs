//! Direct translation of the C++ element filter; symbol names stay aligned for debugging.
...
impl ElementFilter {
    ...
    pub fn AddElement(&mut self, element: &str) {
        if self.m_FilterSpace.is_empty() || self.m_size <= 0 {
            return;
        }
        let n1 = hash(element);
        let n2 = n1 ^ (n1 >> 17);
    ...
    pub fn Contains(&self, element: &str) -> bool {
        ...
        let n2 = n1 ^ (n1 >> 17);
        let size = self.m_size as u64;
        self.m_FilterSpace[(n1 % size) as usize] != 0
            && self.m_FilterSpace[(n2 % size) as usize] != 0
    }
}
