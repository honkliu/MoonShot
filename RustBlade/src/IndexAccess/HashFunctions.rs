...
pub fn MurmurHash3_x86_128(key: &[u8], seed: u32) -> [u32; 4] {
    ...
    for block in key.chunks_exact(16) {
        ...
        k1 = k1.wrapping_mul(C1).rotate_left(15).wrapping_mul(C2); h1 ^= k1;
        h1 = h1.rotate_left(19).wrapping_add(h2).wrapping_mul(5).wrapping_add(0x561ccd1b);
        k2 = k2.wrapping_mul(C2).rotate_left(16).wrapping_mul(C3); h2 ^= k2;
        h2 = h2.rotate_left(17).wrapping_add(h3).wrapping_mul(5).wrapping_add(0x0bcaa747);
        k3 = k3.wrapping_mul(C3).rotate_left(17).wrapping_mul(C4); h3 ^= k3;
        h3 = h3.rotate_left(15).wrapping_add(h4).wrapping_mul(5).wrapping_add(0x96cd1c35);
        k4 = k4.wrapping_mul(C4).rotate_left(18).wrapping_mul(C1); h4 ^= k4;
    ...
    if tail.len() >= 15 { k4 ^= (tail[14] as u32) << 16; }
    if tail.len() >= 14 { k4 ^= (tail[13] as u32) << 8; }
    if tail.len() >= 13 { k4 ^= tail[12] as u32; h4 ^= k4.wrapping_mul(C4).rotate_left(18).wrapping_mul(C1); }
    if tail.len() >= 12 { k3 ^= (tail[11] as u32) << 24; }
    if tail.len() >= 11 { k3 ^= (tail[10] as u32) << 16; }
    if tail.len() >= 10 { k3 ^= (tail[9] as u32) << 8; }
    if tail.len() >= 9 { k3 ^= tail[8] as u32; h3 ^= k3.wrapping_mul(C3).rotate_left(17).wrapping_mul(C4); }
    if tail.len() >= 8 { k2 ^= (tail[7] as u32) << 24; }
    if tail.len() >= 7 { k2 ^= (tail[6] as u32) << 16; }
    if tail.len() >= 6 { k2 ^= (tail[5] as u32) << 8; }
    if tail.len() >= 5 { k2 ^= tail[4] as u32; h2 ^= k2.wrapping_mul(C2).rotate_left(16).wrapping_mul(C3); }
    if tail.len() >= 4 { k1 ^= (tail[3] as u32) << 24; }
    if tail.len() >= 3 { k1 ^= (tail[2] as u32) << 16; }
    if tail.len() >= 2 { k1 ^= (tail[1] as u32) << 8; }
    if !tail.is_empty() { k1 ^= tail[0] as u32; h1 ^= k1.wrapping_mul(C1).rotate_left(15).wrapping_mul(C2); }
...
pub fn MurmurHash3_x64_128(key: &[u8], seed: u32) -> [u64; 2] {
    ...
    for block in key.chunks_exact(16) {
        ...
        k1 = k1.wrapping_mul(C1).rotate_left(31).wrapping_mul(C2); h1 ^= k1;
        h1 = h1.rotate_left(27).wrapping_add(h2).wrapping_mul(5).wrapping_add(0x52dce729);
        k2 = k2.wrapping_mul(C2).rotate_left(33).wrapping_mul(C1); h2 ^= k2;
        h2 = h2.rotate_left(31).wrapping_add(h1).wrapping_mul(5).wrapping_add(0x38495ab5);
    }
    let tail = &key[key.len() / 16 * 16..];
    let (mut k1, mut k2) = (0u64, 0u64);
    for (index, byte) in tail.iter().copied().enumerate().skip(8) { k2 ^= (byte as u64) << ((index - 8) * 8); }
    if tail.len() >= 9 { h2 ^= k2.wrapping_mul(C2).rotate_left(33).wrapping_mul(C1); }
    for (index, byte) in tail.iter().copied().take(8).enumerate() { k1 ^= (byte as u64) << (index * 8); }
    if !tail.is_empty() { h1 ^= k1.wrapping_mul(C1).rotate_left(31).wrapping_mul(C2); }
    let len = key.len() as u64;
    h1 ^= len; h2 ^= len;
    h1 = h1.wrapping_add(h2); h2 = h2.wrapping_add(h1);
    h1 = fmix64(h1); h2 = fmix64(h2);
    h1 = h1.wrapping_add(h2); h2 = h2.wrapping_add(h1);
    [h1, h2]
}

pub fn Hash1(key: &[u8]) -> i32 { MurmurHash3_x86_32(key, 1) as i32 }
pub fn Hash2(key: &[u8]) -> i32 { MurmurHash3_x86_32(key, 2) as i32 }
pub fn HashMurmur3(key: &[u8], seed: i32) -> i32 { MurmurHash3_x86_32(key, seed as u32) as i32 }
pub fn HashCrypto(_key: &[u8]) -> i32 { 0 }
