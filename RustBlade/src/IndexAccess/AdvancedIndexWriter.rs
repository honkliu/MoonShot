//! Direct translation of the C++ advanced writer; symbol names stay aligned for debugging.
...
impl IndexWriter for AdvancedIndexWriter {
    fn Write(&mut self, words: Vec<String>, documentId: u64, postingType: &str) {
        ...
        let mut termTf = StableHashMap::<String, u32>::with_capacity_and_hasher(
            words.len() * 2,
            Default::default(),
        );
        for word in &words {
        ...
        store.AddStreamStats(
            documentId,
            abbrev.as_bytes()[0] as char,
            words.len() as u32,
            uniqueUnigramCount,
        );
    }
